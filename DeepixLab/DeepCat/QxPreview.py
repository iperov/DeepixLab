            ...
            if opened:
                holder.add(
                    qx.QHBox().dispose_with(bag).set_spacing(4)
                        ...
                        .add( qx.QCheckBoxMxFlag(self._preview.mx_patch_mode).set_text('@(Patch_mode)'))
                        .add( patch_mode_holder := qx.QHBox())
                    )

                self._preview.mx_patch_mode.reflect(lambda patch_mode, bag=mx.Disposable().dispose_with(bag):
    ...
    def _ref_patch_mode(self, patch_mode, holder : qx.QHBox, bag : mx.Disposable):
        ...
        if patch_mode:
            holder.add( qx.QHBox().dispose_with(bag)
                            ...
                            .add_spacer(4)
                            .add( qx.QCheckBoxMxFlag(self._preview.mx_fix_borders).set_text('@(Fix_borders)')))
